...
void Scene::addChild(NoriObject *obj) {
	switch (obj->getClassType()) {
		case EMesh:
		case EShape:	//MCO DECLARED LINE
			{
				Mesh *mesh = static_cast<Mesh *>(obj);
				m_kdtree->addMesh(mesh);
				m_meshes.push_back(mesh);

				//Add light meshes into a vector which will be used for explicit MC (at least for now)
				if(mesh->isLight()) {	m_meshLights.push_back(mesh);	}
			}
			break;
